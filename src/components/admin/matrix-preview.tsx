...
export function MatrixPreview({ matrix }: { matrix: AdminMatrixDetail }) {
  return <section className="matrix-preview"><header className="page-header admin-page-header"><div><h1>{matrix.roleName}</h1><p>{matrix.competencies.length} competency requirements</p></div><span className="tag">Version {matrix.version} · {matrix.status}</span></header>

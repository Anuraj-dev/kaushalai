...
export function LearnerJourney() {
  ...
  if (error && !session) return <div className="surface error-state"><div className="alert" role="alert">{error}</div><button className="button button-primary" onClick={() => window.location.reload()}>Try again <span aria-hidden="true">→</span></button></div>;
  if (!session) return <><header className="page-header"><div><h1>Choose an official to assess</h1><p>Use one of the seeded personas to walk through an evidence-based competency assessment.</p></div><span className="tag">Official workspace</span></header><section className="surface picker-surface"><div className="section-intro"><div><span className="tag tag-lime">Seeded officials</span><h2>Choose a working profile</h2></div><p className="muted">Each persona has a different role matrix and learning context.</p></div><div className="official-picker">{officials.map((official) => <button className="official-choice" key={official.id} onClick={() => start(official.id)} disabled={busy}><span className="official-index" aria-hidden="true">{official.id.slice(-2)}</span><span className="official-copy"><strong>{official.name}</strong><span>{official.jobRoleName}</span><small>{official.employeeCode}</small></span><span className="choice-arrow" aria-hidden="true">↗</span></button>)}</div></section></>;
...
function QuestionForm({ questions, answers, setAnswers, onSubmit, busy, round, answered }: { questions: Question[]; answers: Record<string, string>; setAnswers: (value: Record<string, string>) => void; onSubmit: () => void; busy: boolean; round: number; answered: number }) {
  ...
  const roundName = round === 1 ? "Fixed baseline" : round === 2 ? "Personalized evidence" : "Clarification";
  ...
  return <form className="surface assessment-card" onSubmit={(event) => { event.preventDefault(); continueAssessment(); }}>
    ...
    <p className="muted">{round === 1 ? "Choose the statement that best matches your current practice for each competency." : "Short answers help us understand the evidence behind your current level."}</p>
